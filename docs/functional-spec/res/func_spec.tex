...
\hline
% --------------------------------------------------------------------------------------------------------------
Exceptions: & In the case that the User opens a VR title which does not track individual fingers, the User has
mapped a keypress to an individual finger.\\
% --------------------------------------------------------------------------------------------------------------
\end{longtable}
...
Normal flow: &\mbox{}\par\vspace{-\baselineskip}
\begin{enumerate}
\item The User performs several distinct actions while recording is active;
...
% --------------------------------------------------------------------------------------------------------------
Description: &
A sequence of movements is parsed and an attempt to match is made.\\
% --------------------------------------------------------------------------------------------------------------
\hline
% --------------------------------------------------------------------------------------------------------------
Preconditions: &\mbox{}\par\vspace{-\baselineskip}
...
5.a If no match is found in the subsequent searches, the Head item in the input queue is removed from the queue and Step 4
...
\noindent There are several constraints that we foresee will have some impact on the development process of this project.
...
\begin{itemize}
    \item \textbf{Criticality} This Aspect is critical, since, even if the rest of the data is accurately measured, if the data isn't transferred reliably, the device is useless, meaning the entire functionality of the device is dependent on this.
...
\begin{itemize}
    \item \textbf{Criticality:} This aspect is critical to the overall accuracy of the device, especially for tracking the hand for VR and Gesture Recognition, as well as for general use, as it can be very frustrating to move a mouse cursor with a device that doesn't accurately read user inputs.
    \item \textbf{Technical Issues:} A significant issue with this is that the accuracy is almost entirely dependent on the hardware, meaning if the IMU device we use is inaccurate, we won't be able to do anything to improve the tracking.
\end{itemize}
...
\noindent The device must be able to track the fingers' joints accurately.
...
\begin{itemize}
    ...
    \item \textbf{Technical Issues:} Due to the nature of the device, we only have a certain number of inputs, and thereby limited on the amount of information we get from the device, meaning that we will have to focus on some way to distinguish between gestures and non-gestures through pattern recognition.
\end{itemize}
...
\begin{figure}[h!]
    ...
    \noindent The task calls the device driver via a Kernel device call, the driver receives the message from the queue and returns the data to the task and the buffer to the memory partition. When new data arrives, the ISR$^\arabic{Glossary}$ allocates a buffer from the memory partition, puts the data in the buffer, and puts the pointer in a message which is then sent to the queue.
    ...
    \begin{tikzpicture}[
            ...
            every node/.style={align=center}]
        \matrix{%
            \node[source, label=left:Task\\get()] (A) {A}; \& \node[source, label=above:Kernel] (B) {B}; \& \node[source, label=above:Driver get()](C) {C};\\
            \& \& \node[source, label=right:Queue](D){D}; \& \node[source, label=left:Memory\\Partition](E){E};\\ \\
            \& \node[source, label=below:Device](G){G};\& \node[source, label=below:\stepcounter{Glossary}ISR$^\arabic{Glossary}$](F){F};\\
        };

        \node[draw, dotted, fit=(C) (D) (E) (F), inner sep=5ex](box){};
        \node[above of=box] [above=-5ex of box] {Device Driver};

        \draw[>=triangle 45, <->] (A) -- (B);
        \draw[>=triangle 45, <->] (B) -- (C);
        \draw[>=triangle 45, ->] (C) -| (E);
        \draw[>=triangle 45, <-] (C) -- (D);
        \draw[>=triangle 45, <-] (D) -- (F);
...
\begin{figure}
    ...
    \begin{enumerate}
        ...
        \item Decrease in Acceleration on the Z-Axis.
        \item (Acceleration | Deceleration) on Y-Axis with slight deceleration on the Z-Axis.
...
\noindent In this section we will outline our provisional Gantt chart depicting how work will be completed through the duration of this project. The document is laid out as follows:
